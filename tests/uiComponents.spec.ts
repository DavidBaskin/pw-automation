...
test.describe('Form Layouts page', () => {
  ...
  test('input field', async ({page}) => {
    ...
    await usingTheGridEmailInput.pressSequentially('test@test1.com', {delay: 100})
...
test('dropdowns', async({page}) => {
  const dropDownLocatorMenu = page.locator('ngx-header nb-select')
  await dropDownLocatorMenu.click()
...
test('web tables', async ({page}) => {
  ...
  await page.locator('input-editor').getByPlaceholder('E-mail').fill('dava@dada.com')
  await page.locator('.nb-checkmark').click()

  await expect(targetRowId.locator('td').nth(5)).toHaveText('dava@dada.com')
...
test('date picker', async ({page}) => {
  ...
  const formPickerButton = page.getByPlaceholder('Form Picker')
